...
/* --- クイックソート --- */
Sort quick(Member *a, Member *axis, int left, int right, int compare(const Member *y, const Member *z))
{
    int pl = left;
    int pr = right;
    ...
    do
    {
        while (compare(&axis, a + pl) > 0)
            pl++;
        while (compare(a + pr, &axis) > 0)
            pr--;
        if (pl <= pr)
        {
    ...
    if (left < pr){
        Sort l ={a, left, pr, compare};
        return l;
    }
    if (pl < right){
         Sort r ={a, pl, right, compare};
        return r;
    }
}
void sort(Member *a, int left, int right, int compare(const Member *y, const Member *z))
{
    int pl = left;
    int pr = right;
    int size = right - left + 1;
    ...
    if (size >= 5)
    {
        Member axis = *middle3(a + pl, &pm, a + pr, compare);
        Sort quicksort = quick(a, &pm, left, pr, compare);
        sort(&quicksort.a, quicksort.left, quicksort.right , quicksort.compare);
    }
    else
    {
...
int main(void)
{
    ...
    do
    {
        ...
        case ASCEND_NO: /* 番号で昇順にソート */
            sort(data, 0, ndata - 1, AscendingMemberNoCmp);
            break;
        case ASCEND_NAME: /* 名前で昇順にソート */
            sort(data, 0, ndata - 1, AscendingMemberNameCmp);
            break;
        case DESCEND_NO: /* 番号で降順にソート */
            sort(data, 0, ndata - 1, DescendingMemberNoCmp);
            break;
        case DESCEND_NAME: /* 名前で降順にソート */
            sort(data, 0, ndata - 1, DescendingMemberNameCmp);
            break;
